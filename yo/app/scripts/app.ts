...
interface Window {
    registerTopcatPlugin: any;
    deferredBootstrapper: any;
}
...
(function() {
                    ...
                    _.each(config.facilities, function(facility){
                        if(!facility.authenticationTypes){
                            promises.push($.get(facility.icatUrl + "/icat/properties").then(function(properties){
                                facility.authenticationTypes = _.map(properties.authenticators, function(authenticator: any){
                                    return {
                    ...
                    function waitForPlugins(){
                        if(plugins.length == reachablePluginsLength){
                            ...
                            _.each(plugins, function(plugin){
                                ...
                                if(plugin.scripts){
                                    _.each(plugin.scripts, function(scriptUrl: any){
    ...
    app.config(function($stateProvider, $urlRouterProvider, APP_CONFIG){
        ...
        var cookies = {};
        _.each(document.cookie.split(/;\s*/), function(pair: any){
            pair = pair.split(/=/);
